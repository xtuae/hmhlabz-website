...
const Approach = ({ content }) => {
  ...
  const steps = content?.steps?.map((s, i) => ({
    ...s,
    n: `0${i + 1}`,
    tag: i === 0 ? "Diagnose" : i === 1 ? "Recommend" : "Ship",
    out: i === 0 ? "Workflow map" : i === 1 ? "Scored roadmap" : "Live system",
    dark: i === 2
  })) || defaultSteps;
      ...
      <div className="max-w-7xl mx-auto">
        <Reveal className="max-w-4xl mb-20 sm:mb-24 text-left flex flex-col items-start mr-auto">
          ...
          <h2 className="mt-8 font-sans font-bold text-ink leading-[1.04] tracking-[-0.028em]" style={{ fontSize: "clamp(36px, 5.4vw, 64px)" }}>
            {content?.title || "Most studios talk. Most consultants"} <em className="font-serif italic font-normal not-italic text-terra">{content?.highlight || "vanish."}</em>
          </h2>
          <p className="mt-8 max-w-[60ch] font-sans text-[16px] sm:text-[18px] leading-[1.6] text-ink/65">
            {content?.description || "One linear process. The same team carries every step — so nothing gets lost between strategy and shipping."}
          </p>
        </Reveal>
          ...
          {steps.map((s, i) => (
            <Reveal key={s.n} delay={i * 0.1} className={`bg-${s.dark ? 'ink' : 'paper'} text-${s.dark ? 'paper' : 'ink'} rounded-2xl overflow-hidden border border-ink/12 flex flex-col transition-transform hover:scale-[1.02] text-left`}>
              ...
              <div className="px-7 py-9 flex-1 flex flex-col">
                ...
                <h3 className="mt-6 font-sans font-semibold text-[22px] sm:text-[24px] tracking-[-0.01em] leading-[1.2]">{s.title || s.t}</h3>
